...
**[Securities lending](https://www.investopedia.com/terms/s/securitieslending.asp) onchain.**
**Holders** lend out token inventory to **[short sellers](https://www.investopedia.com/terms/s/shortselling.asp)**.
The short seller posts [collateral](https://www.investopedia.com/terms/c/collateral.asp) in a stable
asset (e.g. USDC) and borrows the asset they want to
[short](https://www.investopedia.com/terms/s/short.asp) (e.g. NVDAx — a tokenized NVIDIA share).
They immediately sell the borrowed NVDAx on the open market for USDC, pay a
second-by-second lending fee while the position is open, and later buy equivalent
NVDAx back to return to the holder. If NVDAx's price falls between the sell and the
re-buy, the short seller pockets the difference in USDC; if NVDAx rallies far
enough that their collateral no longer covers the cost of buying it back, keepers
[liquidate](https://www.investopedia.com/terms/l/liquidation.asp) the position.

This is the same primitive that underpins traditional securities lending in TradFi:
holders earn [yield](https://www.investopedia.com/terms/y/yield.asp) on inventory they
would hold anyway (think [exchange-traded funds](https://www.investopedia.com/terms/e/etf.asp),
pension funds, or any passive allocator), and short sellers and
[arbitrageurs](https://www.investopedia.com/terms/a/arbitrageur.asp) get the tokens they
...
1. [Program flow: Alice, Bob, and Carol](#program-flow-alice-bob-and-carol)
2. [What does this program do?](#what-does-this-program-do)
3. [Bilateral versus pooled lending](#bilateral-versus-pooled-lending)
4. [Lifecycle](#lifecycle)
5. [Safety and edge cases](#safety-and-edge-cases)
6. [Running the tests](#running-the-tests)
7. [Quasar port](#quasar-port)
8. [Extending the program](#extending-the-program)

---

## Program flow: Alice, Bob, and Carol

Three participants, two tokenized stocks, one stablecoin.

**NVDAx** is a tokenized NVIDIA share that trades onchain and has a Pyth price feed.
**TSLAx** is the same thing for Tesla. Both are examples of real-world asset tokens —
stocks or bonds issued as Solana tokens so they can be held, transferred, and used in
DeFi programs. **USDC** is a [stablecoin](https://www.investopedia.com/terms/s/stablecoin.asp)
pegged to the US dollar; it is the collateral currency in every example below.

> The same mechanics work for any token with a Pyth price feed. Alice could run a
> parallel TSLAx lease at the same time; a different short seller could take it.
> The program supports multiple concurrent leases per holder via the `lease_id`
> parameter.

### The participants and their motivations

**Alice** holds 100 NVDAx. She would hold NVDAx regardless of whether anyone borrows
it — she has no plans to sell. She lists it as a lease to earn a lending
yield on an asset she would otherwise leave idle. The fee accumulates in
USDC while the lease is active. She never has to sell her NVDAx position
to generate this income; she simply earns from letting others use it
temporarily.

**Bob** has a bearish thesis: he believes NVIDIA is overvalued and its share price
will fall. He wants [short exposure](https://www.investopedia.com/terms/s/shortselling.asp)
— a position that profits when the price drops. Rather than using a
[perpetual futures contract](https://www.investopedia.com/terms/p/perpetual-futures.asp)
(which has no fixed end date and can accrue funding costs unpredictably), Bob wants a
fixed-term, fixed-fee deal so he can model his costs upfront.

**Carol** runs an automated keeper bot. She earns a liquidation bounty by watching
onchain positions for [maintenance margin](https://www.investopedia.com/terms/m/maintenancemargin.asp)
breaches and calling `liquidate` on them. Maintenance margin is the minimum ratio of
collateral value to debt value that a position must maintain — if a position falls
below this floor, the collateral is no longer sufficient to cover the cost of
re-acquiring the borrowed tokens, and the position can be seized. Carol does not need
to know Alice or Bob; she just runs a bot that scans all active leases, checks the
math, and calls `liquidate` whenever one qualifies.

### Step-by-step walkthrough

Assume both USDC and NVDAx use 6 decimal places (1 token = 1,000,000 base units).
NVDAx is trading at $180 per share at the start.

#### Step 1: Alice lists her NVDAx — `create_lease`

Alice calls `create_lease` with the following parameters:

| Parameter | Value | Meaning |
|---|---|---|
| `leased_mint` | NVDAx mint address | The token being lent out |
| `collateral_mint` | USDC mint address | The token Bob must post as security |
| `leased_amount` | `100_000_000` | 100 NVDAx |
| `required_collateral_amount` | `22_000_000_000` | 22,000 USDC — ~122% [loan-to-value](https://www.investopedia.com/terms/l/loantovalue.asp) at spot ($180 × 100 = $18,000 notional; $22,000 / $18,000 ≈ 1.22) |
| `lease_fee_per_second` | `456` | USDC base units / second ≈ 8% [APR](https://www.investopedia.com/terms/a/apr.asp) on $18,000 notional |
| `duration_seconds` | `2_592_000` | 30 days |
| `maintenance_margin_basis_points` | `11_000` | 110% — a [basis point](https://www.investopedia.com/terms/b/basispoint.asp) is 1/100 of a percent, so 11,000 bp = 110%. If the collateral drops below 110% of debt value, the position becomes liquidatable |
| `liquidation_bounty_basis_points` | `100` | 1% of remaining collateral goes to the keeper (Carol) on liquidation |
| `feed_id` | Pyth NVDAx/USD feed id | Pins the exact oracle this lease uses; prevents a keeper from passing a different feed to force a spurious liquidation |

**Accounts created by `create_lease`:**

- `Lease` ([program-derived address](https://solana.com/docs/terminology), seeds `[b"lease", Alice's pubkey, lease_id.to_le_bytes()]`): the state account. Stores every parameter above plus runtime fields (`short_seller`, `collateral_amount`, `start_timestamp`, `end_timestamp`, `last_paid_timestamp`, `status`). Status is set to `Listed`.
- `leased_vault` (PDA, seeds `[b"leased_vault", lease]`): token account for NVDAx. Receives Alice's 100 NVDAx during this call and holds them until a short seller arrives.
- `collateral_vault` (PDA, seeds `[b"collateral_vault", lease]`): token account for USDC. Created empty here; Bob fills it in Step 2.

**Token movement:** 100 NVDAx transfers from Alice's [associated token account](https://solana.com/docs/terminology) (ATA) into `leased_vault`.

After this call Alice's 100 NVDAx are locked in the program-owned vault. She earns nothing yet — the fee clock starts only when Bob (or any short seller) calls `take_lease`.

#### Step 2: Bob takes the position — `take_lease`

Bob finds the `Lease` account onchain (via an indexer or direct lookup) and calls
`take_lease`. This is the instruction that opens the short position.

**Accounts changed by `take_lease`:**

| Account | Before | After |
|---|---|---|
| `lease.status` | `Listed` | `Active` |
| `lease.short_seller` | `Pubkey::default()` | Bob's pubkey |
| `lease.collateral_amount` | `0` | `22_000_000_000` (22,000 USDC) |
| `lease.start_timestamp` | `0` | current time |
| `lease.end_timestamp` | `0` | `start + 30 days` |
| `lease.last_paid_timestamp` | `0` | current time |
| `collateral_vault` | 0 USDC | 22,000 USDC |
| `leased_vault` | 100 NVDAx | 0 NVDAx |
| Bob's USDC ATA | 22,000+ USDC | decremented by 22,000 USDC |
| Bob's NVDAx ATA (`init_if_needed`) | — | 100 NVDAx |

**Token movements (in order, atomic within one transaction):**
1. 22,000 USDC: Bob's USDC ATA → `collateral_vault`
2. 100 NVDAx: `leased_vault` → Bob's NVDAx ATA

Depositing collateral first (step 1 of 2) means that if the leased-token payout
somehow fails, the whole transaction reverts and Bob's collateral is never deducted.

Bob immediately sells the 100 NVDAx on Jupiter for ~$18,000 USDC. He now holds extra
USDC and owes Alice 100 NVDAx. His short position is open. The asset-leasing program
does not perform this swap itself — it and the DEX are separate programs. In practice
a frontend bundles `take_lease` and the Jupiter swap in one transaction so both
succeed or both revert together.

#### Step 3: The lease fee streams — `pay_lease_fee`

While the lease is `Active`, a per-second lending fee accrues out of `collateral_vault`
and toward Alice. The fee is lazy — nothing moves onchain until someone calls
`pay_lease_fee`. Bob has obvious incentive to do so periodically so the running
balance stays settled and he does not get a surprise [margin call](https://www.investopedia.com/terms/m/margincall.asp).
The call is permissionless: Carol's bot can also trigger it.

**Accounts changed by `pay_lease_fee`:**

| Account | Change |
|---|---|
| `lease.collateral_amount` | decremented by fee paid |
| `lease.last_paid_timestamp` | updated to `min(now, end_timestamp)` |
| `collateral_vault` | USDC balance decremented by fee paid |
| `holder_collateral_account` (Alice's USDC ATA, `init_if_needed`) | USDC balance incremented by fee paid |

If Bob's collateral ratio drifts toward the 110% maintenance-margin floor as NVDAx's
price rises, he can call `top_up_collateral` to add more USDC.

**Accounts changed by `top_up_collateral`:**

| Account | Change |
|---|---|
| `lease.collateral_amount` | incremented by the top-up amount |
| `collateral_vault` | USDC balance incremented |
| `short_seller_collateral_account` (Bob's USDC ATA) | USDC balance decremented |

#### Step 4a: NVIDIA falls to $160 — Bob profits — `return_lease`

Bob's thesis plays out. He buys 100 NVDAx back on Jupiter for ~$16,000 USDC
(less than the $18,000 he received when he sold them) and calls `return_lease`.

**Accounts changed (then closed) by `return_lease`:**

| Account | Change |
|---|---|
| `lease.status` | `Active → Closed` |
| `lease.collateral_amount` | set to `0` before close |
| `lease.last_paid_timestamp` | updated to `min(now, end_timestamp)` before close |
| Bob's NVDAx ATA | decremented by 100 NVDAx |
| `leased_vault` | receives 100 NVDAx from Bob; immediately forwarded to Alice (vault then closed) |
| Alice's NVDAx ATA (`init_if_needed`) | incremented by 100 NVDAx |
| `collateral_vault` | drained: fees to Alice, remainder to Bob (vault then closed) |
| Alice's USDC ATA | incremented by accrued USDC fees |
| Bob's USDC ATA | incremented by remaining USDC collateral (after fees) |
| `lease` account | closed; rent-exempt lamports returned to Alice |

**Token movements (in order):**
1. 100 NVDAx: Bob's NVDAx ATA → `leased_vault` → Alice's NVDAx ATA
   (the leased vault is an intermediary hop so the program can reuse the vault's PDA
   signing path; the round-trip leaves it empty and ready to close)
2. Accrued USDC fees: `collateral_vault` → Alice's USDC ATA
3. Remaining USDC: `collateral_vault` → Bob's USDC ATA

Example at T + 600 seconds (10 minutes elapsed):
- Accrued fee: 600 s × 456 USDC base units/s = 273,600 USDC base units ≈ 0.27 USDC
- Bob's USDC refund: 22,000 − 0.27 ≈ 21,999.73 USDC
- Bob's gross profit ≈ $18,000 (sale) − $16,000 (buy-back) − 0.27 (fee) − trading costs ≈ **$1,999.73**

Alice receives: her 100 NVDAx back plus the USDC lending fees she earned.

#### Step 4b: NVIDIA rallies to $200 — Carol liquidates — `liquidate`

Bob's thesis is wrong. NVDAx rises to $200 per share.

- Cost to re-acquire 100 NVDAx (the debt): 100 × $200 = **$20,000**
- Collateral value: **$22,000** USDC (minus any fees already paid)
- Collateral ratio: $22,000 / $20,000 = **110%** — exactly at the maintenance-margin floor
- One tick higher and the position is liquidatable

Carol's keeper bot detects this, fetches a fresh Pyth price update (the update must
be no older than 60 seconds), and calls `liquidate`.

The program first verifies:
1. The Pyth account's `discriminator` and byte layout are valid
2. The `feed_id` in the price update matches `lease.feed_id` (the feed-pinning guard — without it a keeper could supply a random unrelated feed to force a spurious liquidation)
3. `publish_time` is within 60 seconds of now
4. The position is genuinely underwater (`collateral_value × 10,000 < debt_value × maintenance_margin_basis_points`)

**Accounts changed (then closed) by `liquidate`:**

| Account | Change |
|---|---|
| `lease.status` | `Active → Liquidated` |
| `lease.collateral_amount` | set to `0` before close |
| `collateral_vault` | drained: fee to Alice, bounty to Carol, remainder to Alice (vault closed) |
| `leased_vault` | already empty (Bob kept the NVDAx); closed |
| Alice's USDC ATA (`init_if_needed`) | incremented by fee + holder share |
| Carol's USDC ATA (`init_if_needed`) | incremented by liquidation bounty |
| `lease` account | closed; rent-exempt lamports returned to Alice |

**USDC flows at T + 300 seconds (5 minutes elapsed):**
1. Accrued fee (300 × 456 = 136,800 base units ≈ 0.14 USDC) → Alice's USDC ATA
2. Bounty (1% of remaining ~21,999.86 USDC ≈ **220 USDC**) → Carol's USDC ATA
3. Remainder (~21,779.86 USDC) → Alice's USDC ATA

Bob loses his $22,000 USDC collateral. He still holds 100 NVDAx (now worth $20,000),
so his net loss is the $2,000 difference between what his collateral was worth and the
current NVDAx price, plus fees.
...
**token B** — the collateral token — to take delivery of the A tokens.
...
[program-derived address](https://solana.com/docs/terminology) signs
all the transfers in and out. There is no admin key and no off-program
logic that can move funds: every transfer is dictated by the rules
below, and those rules are the deployed bytecode. Specifically:
...
expressed in [basis points](https://www.investopedia.com/terms/b/basispoint.asp)
(1 basis point = 1/100 of a percent), of required collateral value to
debt value. `maintenance_margin_basis_points = 12_000` is 120%,
meaning the collateral must stay worth at least 1.2× the borrowed
...
[liquidity pool](https://www.investopedia.com/terms/l/liquidity-pool.asp)
that borrowers draw from against collateral, with rates set
automatically by a utilisation curve. That design works well for some
assets and poorly for others. We chose bilateral lending — direct
deals between one holder and one short seller — and it's worth
explaining why.
