...
pub struct LiquidateAccountConstraints<'info> {
    ...
    /// Holder's collateral-mint ATA, created by the keeper if absent. The holder is a
    /// non-signer here so they cannot be the payer. Requiring a pre-existing ATA would
    /// let a holder without one block their own liquidation, trapping the short_seller's
...
pub fn handle_liquidate(context: Context<LiquidateAccountConstraints>) -> Result<()> {
    // PriceUpdateV2 carries both `publish_time` (a unix timestamp marking when
    // the price was observed) and `posted_slot` (when the receiver wrote it
    // onchain). We gate staleness on `publish_time` — the price's own freshness
    // — so we compare against the unix clock to stay in the oracle's own units.
    ...
    // Pre-compute all amounts before any mutable borrows.
