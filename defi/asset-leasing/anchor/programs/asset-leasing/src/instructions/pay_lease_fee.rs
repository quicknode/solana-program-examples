...
#[derive(Accounts)]
pub struct PayLeaseFeeAccountConstraints<'info> {
    ...
    /// Holder's collateral-mint ATA, created on demand by the (permissionless) caller.
    /// The holder is a non-signer here so they cannot pay; the caller funds the ATA
    /// rent and is immediately compensated: the rent-exempt lamports come back when the
    /// holder closes their ATA. Requiring a pre-existing ATA would let a holder without
    /// one block all fee payments on their own lease.
    #[account(
        init_if_needed,
        payer = payer,
...
pub fn handle_pay_lease_fee(context: Context<PayLeaseFeeAccountConstraints>) -> Result<()> {
    ...
    if payable > 0 {
        ...
        // Update state before the token transfer CPI (Checks-Effects-Interactions).
...
impl<'info> PayLeaseFeeAccountConstraints<'info> {
