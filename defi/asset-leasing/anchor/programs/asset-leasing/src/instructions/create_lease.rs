...
pub struct CreateLeaseAccountConstraints<'info> {
    #[account(mut)]
...
pub fn handle_create_lease(
    context: Context<CreateLeaseAccountConstraints>,
    lease_id: u64,
    leased_amount: u64,
    ...
    // Capture keys and bumps before taking a mutable borrow of the lease account.
    let holder_key = context.accounts.holder.key();
    let leased_mint_key = context.accounts.leased_mint.key();
    let collateral_mint_key = context.accounts.collateral_mint.key();
    let lease_bump = context.bumps.lease;
    let leased_vault_bump = context.bumps.leased_vault;
    let collateral_vault_bump = context.bumps.collateral_vault;
