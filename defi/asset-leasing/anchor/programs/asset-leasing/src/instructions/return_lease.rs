...
#[derive(Accounts)]
pub struct ReturnLeaseAccountConstraints<'info> {
    #[account(mut)]
    ...
    /// Holder's leased-mint ATA, created on demand by the short_seller. The holder is a
    /// non-signer here and cannot be the payer. Rent is nominal; requiring the holder to
    /// pre-create it would let a missing ATA block their own settlement and the short_seller's
    /// collateral refund.
    #[account(
        init_if_needed,
        payer = short_seller,
    ...
    /// Holder's collateral-mint ATA — same rationale as holder_leased_account above.
    #[account(
        init_if_needed,
        payer = short_seller,
        associated_token::mint = collateral_mint,
...
pub fn handle_return_lease(context: Context<ReturnLeaseAccountConstraints>) -> Result<()> {
