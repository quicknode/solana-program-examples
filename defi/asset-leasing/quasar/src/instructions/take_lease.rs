...
#[inline(always)]
pub fn handle_take_lease(accounts: &mut TakeLeaseAccountConstraints) -> Result<(), ProgramError> {
    let now = <Clock as quasar_lang::sysvars::Sysvar>::get()?.unix_timestamp.get();

    // Pre-capture all values needed before taking the mutable borrow.
    ...
    let lessee_address = *accounts.lessee.address();
