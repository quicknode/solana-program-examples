...
pub struct OrderBook {
    /// Market PDA this book belongs to. Constrained on-chain via the
...
impl OrderBook {
    ...
    pub fn remove_from(&mut self, side: OrderSide, order_id: u64) -> Option<LeafNode> {
        let key = new_node_key(side, /*price*/ 0, order_id);
        // The seq_num half of the key is order_id (or its bitwise NOT on
        // bids), so we can reconstruct it without knowing the price. But we
        // do still need the price for the high bits; tree search keys on
        // exact match so we have to walk the leaves to find it.
        let _ = key;
        let (root, nodes) = match side {
            OrderSide::Bid => (&mut self.bids_root, &mut self.bids),
            OrderSide::Ask => (&mut self.asks_root, &mut self.asks),
