...
impl Env {
    pub fn new() -> Self {
        ...
        // The market is seeded by (owner, market_id), so one owner can run
        // several markets. This is the owner's market 0.
        let market_id: u64 = 0;
        let market = pda(&[LENDING_MARKET_SEED, owner.pubkey().as_ref(), &market_id.to_le_bytes()]);
