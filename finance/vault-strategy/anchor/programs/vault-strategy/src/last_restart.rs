...
//!
//! Why the program reads it: the vault checks Pyth freshness in seconds, and
//! under Alpenglow each leader sets the Clock's `unix_timestamp`, which may
//! advance by at most twice the slot time elapsed since the parent block. A
//! halt barely advances the slot count, so after a restart the timestamp
//! cannot jump forward by the length of the halt: it trails real time and
//! catches up gradually. A price published just before the halt can therefore
//! still look fresh by seconds. `oracle::load_price` rejects any price posted
//! at or before the restart slot, so deposits and rebalances pause until Pyth
//! posts again.
