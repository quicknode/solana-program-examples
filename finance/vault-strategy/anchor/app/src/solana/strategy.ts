...
export async function loadStrategyView(
  ...
  const assets: AssetView[] = configs.map((c, i) => {
    ...
    if (!c) {
      navComplete = false
      return {
        index: i, config, mint: config, vault: config, priceFeed: config,
        weightBps: 0, vaultAmount: 0n, price: null, publishTime: null,
        stale: false, valueUsdc: null, actualWeight: null,
      }
    }
