...
function RebalancePanel({ view, onRebalance }: { view: StrategyView; onRebalance: VaultState['rebalance'] }) {
  ...
  const legStale =
    sellAsset.price === null || sellAsset.stale || buyAsset.price === null || buyAsset.stale
    ...
    <Panel title="Set weight" hint="0% retires the asset">
      ...
      <TextField
        ...
        right={
          <button
            type="button"
            onClick={() => setPct('0')}
            className="transition-colors hover:text-loss"
          >
            Retire → 0%
          </button>
        }
      ...
      {newBps !== null && !invalid && (
        <p className="font-mono text-[12px] text-faint">
          Resulting total{' '}
          <span className="text-muted">{formatBps(newTotal, 0)}</span>{' '}
      ...
      <Button
        disabled={!ready || busy}
        onClick={() => run(() => onSetWeight(assetIdx, newBps!), 'Weight change')}
      >
...
function AddAssetPanel({ view, onAddAsset }: { view: StrategyView; onAddAsset: VaultState['addAsset'] }) {
  ...
  return (
    <Panel title="Add asset" hint={`${view.account!.assetCount}/${MAX_ASSETS} assets`}>
      ...
      <Button
        disabled={!ready || busy}
        onClick={() => run(() => onAddAsset(mint!, newBps!), 'Add asset')}
      >
...
function CollectFeesPanel({ view, onCollect }: { view: StrategyView; onCollect: VaultState['collectFees'] }) {
  ...
  const feeShares =
    (view.totalShares * BigInt(s.feeBps) * elapsed) / (BigInt(BPS_DENOMINATOR) * SECONDS_PER_YEAR)
    ...
    <div className="mx-auto mt-6 max-w-xl">
      <Panel title={`Create strategy #${STRATEGY_INDEX.toString()}`} hint="manager setup">
        <p className="text-[12px] leading-relaxed text-muted">
          Bootstraps this strategy at index {STRATEGY_INDEX.toString()}. Requires a registry a
          curator has already created and approved assets in. You become the manager.
        </p>
        <TextField label="Registry" value={registry} onChange={setRegistry} placeholder="Base58…" invalid={registry.trim() !== '' && !registryPk} />
        <TextField label="USDC mint" value={usdc} onChange={setUsdc} placeholder="Base58…" invalid={usdc.trim() !== '' && !usdcPk} />
        <TextField label="Swap router program" value={router} onChange={setRouter} placeholder="Base58…" invalid={router.trim() !== '' && !routerPk} />
        <div className="grid grid-cols-2 gap-3">
