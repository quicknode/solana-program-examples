...
function buildSegments(view: StrategyView): Segment[] {
  ...
  view.assets.forEach((a, i) => {
    const pct = useActual ? (a.valueUsdc !== null ? Number(a.valueUsdc) / nav : 0) : a.weightBps / 10_000;
    if (pct > 0) {
...
export function AllocationPanel({ view }: { view: StrategyView }) {
  ...
  return (
    <section className="px-6 py-6">
      ...
      {segments.length > 0 && (
        <div
          className="mb-6 flex h-2.5 w-full gap-px overflow-hidden bg-graphite"
          role="img"
          aria-label={`allocation by ${basisLabel}`}
        >
          {segments.map((seg) => (
            <div
              key={seg.key}
              style={{ width: `${seg.pct * 100}%`, backgroundColor: seg.color }}
              title={`${seg.label} · ${formatRatioPct(seg.pct)}`}
            />
          ))}
        </div>
      )}
