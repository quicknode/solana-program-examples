...
if (program) {
  ...
  for (const ix of idl.instructions) {
    try {
      const accounts = Object.fromEntries(ix.accounts.map((a) => [snakeToCamel(a.name), Keypair.generate().publicKey]));
...
const strategy = PublicKey.findProgramAddressSync([Buffer.from("strategy"), u64le(0)], PROGRAM_ID)[0];
const shareMint = PublicKey.findProgramAddressSync([Buffer.from("share_mint"), strategy.toBuffer()], PROGRAM_ID)[0];
