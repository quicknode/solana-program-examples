...
// time-in-force orders). Error type is rebased to this crate's
// `OrderBookError`. The tree mechanics (critbit / radix-trie insert + remove,
// min/max walks) are preserved so future contributors can diff against
// upstream cleanly. Two write-only traversal stacks upstream carried have been
// dropped: this program is `no_std` with no heap, and neither stack was ever
// read.
...
const_assert_eq!(core::mem::size_of::<OrderTreeRoot>(), 8);
...
const_assert_eq!(
    core::mem::size_of::<OrderTreeNodes>(),
...
impl OrderTreeNodes {
    ...
    fn alloc(&mut self, val: &AnyNode) -> Result<NodeHandle, ProgramError> {
        match NodeTag::from_u8(val.tag) {
            Some(NodeTag::InnerNode) | Some(NodeTag::LeafNode) => (),
            _ => return Err(OrderBookError::OrderBookFull.into()),
        };

        if self.free_list_len == 0 {
            if !((self.bump_index as usize) < self.nodes.len() && self.bump_index < u32::MAX) {
                return Err(OrderBookError::OrderBookFull.into());
            }
        ...
        loop {
            ...
            if let Some(NodeRef::Inner(inner)) = parent_contents.case() {
                if shared_prefix_len >= inner.prefix_len {
                    ...
                    // descend.
                    let (child, _crit_bit) = inner.walk_down(new_leaf.key);
                    parent_handle = child;
                    continue;
                }
            }
