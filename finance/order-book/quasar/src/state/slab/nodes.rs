...
//
// This port covers fixed-price-only orders (no oracle-pegged prices, no
// time-in-force / expiry). Order identity lives in the onchain `Order` PDA
// rather than inline fields. The slab/tree mechanics (NodeTag,
// InnerNode/LeafNode/FreeNode, AnyNode, walk_down, new_node_key) are
// preserved as-is so future contributors can diff against upstream cleanly.
...
///
/// `owner` is the raw 32-byte address (not Quasar's `Address`) so `LeafNode`
/// stays a plain `bytemuck::Pod`; the handler converts to/from `Address` at
/// the boundary.
///
...
pub struct LeafNode {
    ...
    /// Owner of this resting order (raw 32-byte address). Same as the
    /// corresponding `Order` account's `owner`; cached here so the matching
    /// loop doesn't have to deserialize the maker account just to read the
    /// owner.
    pub owner: [u8; 32],

    /// Quantity remaining (in base lots). Decremented as fills consume the
...
impl LeafNode {
    pub fn new(key: u128, owner: [u8; 32], quantity: u64, order_id: u64, timestamp: i64) -> Self {
        Self {
