...
pub fn handle_cancel_prize(context: Context<CancelPrize>, _prize_index: u8) -> Result<()> {
    ...
    // Checks-effects-interactions: lock the prize before draining and closing
    // the vault, so the prize is settled regardless of what the token program
    // does during the CPIs below.
