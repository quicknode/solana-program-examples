...
pub struct AddPrize<'info> {
    ...
    // works for the Classic Token Program (e.g. USDC) and the Token Extensions
    // Program.
...
pub fn handle_add_prize(context: Context<AddPrize>, amount: u64) -> Result<()> {
    // A zero-amount prize can never pay out anything, so reject it at
    // creation rather than letting it sit unpayable.
    require!(amount > 0, HackathonError::ZeroAmount);
