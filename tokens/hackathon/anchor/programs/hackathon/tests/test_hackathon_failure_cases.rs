...
#[test]
fn add_prize_fails_when_amount_is_zero() {
    let mut world = setup_world();
    let (hackathon, _) = hackathon_pda(&world.committee.vault, HACKATHON_NAME);
    let create_ix = create_hackathon_instruction(
        &world.committee.vault,
    ...
    // A zero-amount prize must be rejected by the `ZeroAmount` guard, so the
    // inner add_prize fails and `vault_transaction_execute` reverts. We catch
    // the panic from `run_through_multisig`'s `.expect()` on the failing
    // execute step rather than letting it abort the test.
    let (prize, _) = prize_pda(&hackathon, 0);
    let vault = prize_vault_address(&prize, &world.mint);
    let add_ix = add_prize_instruction(
        &world.committee.vault,
    ...
    assert!(
        result.is_err(),
        "add_prize with amount 0 must be rejected by the ZeroAmount guard"
    );
}
