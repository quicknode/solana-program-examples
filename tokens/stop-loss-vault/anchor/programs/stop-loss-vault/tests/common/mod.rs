...
/// SOL has 9 decimals (lamports per SOL).
...
pub fn new_world() -> TestWorld {
    ...
    let volatile_mint = create_mint(&mut svm, &mint_authority, SOL_DECIMALS);
...
// Silence `unused_imports` complaints when individual test binaries don't
// touch every helper.
#[allow(dead_code)]
pub fn _silence_unused() -> usize {
    // The `Pack` trait import is genuinely unused now \u2014 we read token
    // balances by offset. Kept the bring-in for symmetry with future tests
    // that want to read full account state.
    0
}
