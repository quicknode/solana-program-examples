...
/// when a Switchboard On-Demand price feed reports a price at or below
...
pub struct Vault {
    ...
    /// Suggested crank cadence in seconds. The onchain program does NOT
    ...
    /// Set to `true` after `convert_if_triggered` fires. Locks the pre-trigger
    /// actions (`deposit`, `update_threshold`, `withdraw_volatile`) so the
    /// post-trigger vault is simply a stable-token wallet drained via
    /// `withdraw_stables`.
    pub triggered: bool,
