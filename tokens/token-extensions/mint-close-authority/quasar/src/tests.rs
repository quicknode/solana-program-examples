use {
    crate::cpi::{CloseInstruction, InitializeInstruction},
    quasar_test::prelude::*,
};
...
/// Initialize creates a mint with the MintCloseAuthority extension, and close
/// then reclaims its lamports to the authority.
#[quasar_test]
fn initialize_then_close_reclaims_the_mint(test: &mut Test) {
    ...
    // + 32 MintCloseAuthority bytes = 202 bytes.
    assert_eq!(mint.data.len(), 202);

    test.send(CloseInstruction {
        authority: PAYER,
        mint_account: MINT,
    })
    .succeeds()
