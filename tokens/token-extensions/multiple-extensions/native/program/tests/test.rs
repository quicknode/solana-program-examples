use {
    ...
    token_2022_multiple_extensions_program::CreateTokenArgs,
};

const RENT_SYSVAR_ID: Pubkey = pubkey!("SysvarRent111111111111111111111111111111111");

#[test]
fn test_create_token_with_multiple_extensions() {
    ...
    let program_bytes =
        include_bytes!("../../../../../../target/deploy/token_2022_multiple_extensions_program.so");
    ...
    // NonTransferable has no fields; its presence is what we assert.
    state
        .get_extension::<NonTransferable>()
        .expect("NonTransferable extension should be present");
}
