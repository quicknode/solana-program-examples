use {
    litesvm::LiteSVM,
    nft_minter_program::instructions::create::CreateTokenArgs,
...
#[test]
fn test_create_and_mint_nft() {
    let mut svm = LiteSVM::new();

    let program_id = Pubkey::new_unique();
    svm.add_program(
        program_id,
        include_bytes!("../../tests/fixtures/nft_minter_program.so"),
    )
    .unwrap();
    svm.add_program(
    ...
    // --- Create NFT mint + metadata (Create = variant 0) ---
    ...
    assert_eq!(Mint::unpack(&mint_account.data).unwrap().decimals, 0);
    assert_eq!(
        svm.get_account(&metadata).unwrap().owner,
        TOKEN_METADATA_PROGRAM_ID
    );

    // --- Mint the NFT + create master edition (Mint = variant 1, unit) ---
    ...
    // The ATA holds 1 NFT.
    let token_state = TokenAccount::unpack(&svm.get_account(&ata).unwrap().data).unwrap();
    assert_eq!(token_state.amount, 1);
    assert_eq!(token_state.mint, mint.pubkey());

    // The master edition account exists and is owned by Token-Metadata.
    assert_eq!(
