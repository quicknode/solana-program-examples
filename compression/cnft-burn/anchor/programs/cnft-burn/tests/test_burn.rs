//! LiteSVM integration test for the cnft-burn Anchor program.
//!
//! Full flow exercised:
//!   1. Load the cnft-burn program plus the three mainnet fixtures
...
// Anchor-style instruction discriminator for `burn_cnft` = sha256("global:burn_cnft")[..8].
// Computed in `burn_cnft_disc()` below to avoid a sha2 dependency-mismatch; the program
// is generated by Anchor so we mirror that scheme.
...
// ---- Anchor discriminator for burn_cnft ------------------------------------

fn burn_cnft_disc() -> [u8; 8] {
    // sha256("global:burn_cnft")[..8]. Implemented inline to avoid pulling a
    // crypto crate that conflicts with the program's solana version.
    let digest = sha256(b"global:burn_cnft");
    let mut out = [0u8; 8];
...
fn test_burn_cnft() {
    ...
    svm.add_program(
        CNFT_BURN_ID,
        include_bytes!("../../../target/deploy/cnft_burn.so"),
    )
    .unwrap();
    svm.add_program(
        BUBBLEGUM_ID,
    ...
    // Build the MetadataArgs for the single cNFT we mint.
    let creator = Creator {
        address: leaf_owner.pubkey().to_bytes(),
        verified: false,
        share: 100,
    };
    let metadata = MetadataArgs {
        name: "Test cNFT".to_string(),
    ...
    // Build burn_cnft via our program. Accounts per BurnCnft struct:
    // leaf_owner (signer,mut), tree_authority (mut), merkle_tree (mut),
    // log_wrapper, compression_program, bubblegum_program, system_program,
    // then proof nodes as remaining accounts.
    let mut burn_accounts = vec![
        AccountMeta::new(leaf_owner.pubkey(), true),
        AccountMeta::new(tree_config, false),
        AccountMeta::new(merkle_tree.pubkey(), false),
        AccountMeta::new_readonly(NOOP_ID, false),
        AccountMeta::new_readonly(COMPRESSION_ID, false),
    ...
    for node in proof.iter() {
        burn_accounts.push(AccountMeta::new_readonly(
    ...
    let burn_data = {
        let mut d = burn_cnft_disc().to_vec();
    ...
    let burn_ix = Instruction {
        program_id: CNFT_BURN_ID,
        accounts: burn_accounts.clone(),
...
#[test]
fn test_burn_cnft_disc_matches_program() {
    // Sanity: the Anchor discriminator we compute equals sha256("global:burn_cnft")[..8].
    let disc = burn_cnft_disc();
    assert_eq!(disc.len(), 8);
    // Also exercise our empty_node against the known depth-1 value.
    let e1 = empty_node(1);
