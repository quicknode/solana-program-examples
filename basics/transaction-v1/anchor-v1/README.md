...
See also the [Anchor v2](../anchor/), [Pinocchio](../pinocchio/) and [native Rust](../native/) variants of the same program, and [docs/transaction-v1.md](../../../docs/transaction-v1.md) for what v1 changes, its activation status, and which tools support it.
...
**The document account.** `Document`, a [PDA](https://solana.com/docs/references/terminology#program-derived-account-pda) with seeds `["document", payer]`, one per payer. There is no `InitSpace`: `Document::required_space` sizes the account to each document (discriminator, borsh length prefix, bytes), and the payer pays that [rent](https://solana.com/docs/references/terminology#rent).
...
From this directory (`basics/transaction-v1/anchor-v1/`):

```bash
anchor build
```

Prerequisites: [Agave](https://docs.anza.xyz/) CLI (version in `Anchor.toml` `[toolchain]`; the LiteSVM tests carry their own v1-capable runtime, so the CLI version does not matter to them), [Anchor](https://www.anchor-lang.com/docs) v1.

Like every `anchor-v1/` directory, this example is its own Cargo workspace outside the repository root workspace.
...
This runs `cargo test` as configured in `Anchor.toml`. LiteSVM 0.16.0 is the first release that executes v1 transactions, and this example builds its transactions by hand rather than with `solana-kite`, which the other Anchor v1 examples use: kite's `send_transaction_from_instructions` builds a legacy transaction. The suite covers:
