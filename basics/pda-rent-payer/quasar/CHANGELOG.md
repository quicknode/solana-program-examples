...
- Migrated to Quasar 0.1.0 (`0.1.0-release` branch, rev `be60fca`): Quasar.toml
  ...
  assertions were dropped pending recalibration under 0.1.0. One program-source
  fix: `Seed` left the 0.1.0 prelude, so `create_new_account.rs` now imports
  `quasar_lang::cpi::Seed` explicitly.
